...
function make(source:string,title:string,company:string,location:string,description:string,url:string,type="Não informado",tags:string[]=[]):Job|null{
  title=clean(title); description=clean(description); if(!title||!IT.test(title)||/^(sales|solutions partner)\b/i.test(title)||!url)return null;
  const key=`${title}|${company}|${location}`.toLowerCase();
  const country=!location||location==="Não informado"?"Não informado":/brasil|brazil|\bbr\b|\bbra\b|s[aã]o paulo|rio de janeiro|campinas|curitiba|bras[ií]lia|bauru|fortaleza|recife|salvador|belo horizonte|porto alegre|goi[aâ]nia|manaus|florian[oó]polis/i.test(location)?"Brasil":"Exterior";
...
async function workable(source:typeof SOURCES[number]){
  const r=await fetch(`https://apply.workable.com/api/v1/widget/accounts/${source.slug}`,{headers:{accept:"application/json"},cache:"no-store",signal:AbortSignal.timeout(12000)});
  if(!r.ok)throw new Error(`${source.name}: ${r.status}`);
...
async function gupy(source:typeof SOURCES[number]){
  const r=await fetch(`https://${source.slug}.gupy.io/api/job_postings`,{headers:{accept:"application/json"},cache:"no-store",signal:AbortSignal.timeout(12000)});
  if(!r.ok)throw new Error(`${source.name}: ${r.status}`);
